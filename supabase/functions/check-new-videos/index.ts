...
// Fetch with timeout
async function fetchWithTimeout(url: string, options = {}, timeout = FETCH_TIMEOUT_MS): Promise<Response> {
...
async function fetchYouTubeTranscript(videoId: string): Promise<string> {
  ...
  try {
    ...
    const response = await fetchWithTimeout(`https://youtube-transcript3.p.rapidapi.com/api/transcript?videoId=${videoId}`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to fetch transcript: ${response.status} ${response.statusText} - ${errorBody}`);
    }
    ...
    const transcriptText = data.transcript.map((segment: { text: string }) => segment.text).join(' ');
  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('Summary:')) {
        ...
        people = peopleString.split(',').map((person: string) => person.trim()).filter((person: string) => person.length > 0);
    ...
    throw new Error(`Error generating summary: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function extractVideoLink(entry: any): string {
  ...
  if (entry.link) {
    ...
    if (typeof entry.link === 'string') {
      ...
      const alternateLink = entry.link.find((l: any) => l['@attributes'] && l['@attributes'].rel === 'alternate');
...
async function handleRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    const { data: allChannels, error: channelsError } = await supabase.from('channels').select('id, name, rss_feed_url, last_checked').order('last_checked', {
      ascending: true,
      nullsFirst: true
    });
    if (channelsError) {
    ...
    channelsToProcess.forEach((channel: Channel) => {
      ...
      try {
        ...
        for (const entry of entriesToProcess){
          try {
            ...
            const { data: existingSummary, error: summaryCheckError } = await supabase.from('summaries').select('id, title, summary, tags, featured_names, publisher_name, content_created_at').eq('content_id', currentVideoId).maybeSingle();
            ...
            try {
              ...
              if (transcript.length > 0) {
                ...
                const { data: insertedData, error: summaryInsertError } = await supabase.from('summaries').insert([
                  {
                ...
                const { error: updateError } = await supabase.from('summaries').update({
                  summary: summary,
                  tags: tags,
